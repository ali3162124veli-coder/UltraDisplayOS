...
Result<void> WasapiCapture::init() {
    ...
    if (FAILED(hr)) return ud::Error(ud::ErrorCode::SystemError, "Failed to create IMMDeviceEnumerator");

    hr = enumerator->GetDefaultAudioEndpoint(eRender, eConsole, &device_);
    if (FAILED(hr)) return ud::Error(ud::ErrorCode::SystemError, "Failed to get default audio endpoint");

    hr = device_->Activate(__uuidof(IAudioClient), CLSCTX_ALL, nullptr, &audio_client_);
    if (FAILED(hr)) return ud::Error(ud::ErrorCode::SystemError, "Failed to activate IAudioClient");

    WAVEFORMATEX* mix_format = nullptr;
    hr = audio_client_->GetMixFormat(&mix_format);
    if (FAILED(hr)) return ud::Error(ud::ErrorCode::SystemError, "Failed to get mix format");
    ...
    if (FAILED(hr)) {
        CoTaskMemFree(mix_format);
        return ud::Error(ud::ErrorCode::SystemError, "Failed to initialize IAudioClient for loopback");
    }
    CoTaskMemFree(mix_format);

    event_handle_ = CreateEvent(nullptr, FALSE, FALSE, nullptr);
    if (!event_handle_) return ud::Error(ud::ErrorCode::SystemError, "Failed to create event handle");

    hr = audio_client_->SetEventHandle(event_handle_);
    if (FAILED(hr)) return ud::Error(ud::ErrorCode::SystemError, "Failed to set event handle");

    hr = audio_client_->GetService(IID_PPV_ARGS(&capture_client_));
    if (FAILED(hr)) return ud::Error(ud::ErrorCode::SystemError, "Failed to get IAudioCaptureClient service");

    return Result<void>();
}

Result<void> WasapiCapture::start() {
    if (is_capturing_) return Result<void>();
    HRESULT hr = audio_client_->Start();
    if (FAILED(hr)) return ud::Error(ud::ErrorCode::SystemError, "Failed to start audio client");
    is_capturing_ = true;
    return Result<void>();
}
...
Result<std::vector<float>> WasapiCapture::read_frames() {
    if (!is_capturing_) return ud::Error(ud::ErrorCode::SystemError, "Not capturing");

    DWORD wait_res = WaitForSingleObject(event_handle_, 1000);
    if (wait_res != WAIT_OBJECT_0) return ud::Error(ud::ErrorCode::SystemError, "Wait for audio event failed");

    UINT32 packet_length = 0;
    HRESULT hr = capture_client_->GetNextPacketSize(&packet_length);
    if (FAILED(hr)) return ud::Error(ud::ErrorCode::SystemError, "Failed to get next packet size");
